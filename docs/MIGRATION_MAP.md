...
| Artifact status/truth semantics across standards, schemas, templates and runtime | MDS legacy/current | `docs/foundation/artifact-truth-model.md`, then `mds-core/standards/` plus runtime domain/validation contracts | `MERGE` | `IN_PROGRESS` | The truth model was human-approved on 2026-08-14. `validity_state`, lineage and Current Project Truth still require versioned canonical/runtime migration. Detailed lineage storage Option A is proposed in `docs/foundation/found-009-lineage-storage-adr.md`; runtime work remains blocked until architecture approval. |
