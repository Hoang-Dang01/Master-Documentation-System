...
Di chuyển vào thư mục `projects/active/` và soạn thảo 4 file ngữ cảnh nền tảng:
...
Đừng mở toàn bộ thư mục cùng lúc. Hãy truy cập vào **[views/solo_view.md](views/solo_view.md)**:
...
1.  Mở thư mục **[ai_skills/agents/](ai_skills/agents/)** và tìm System Prompt tương ứng cho Agent đó.
...
3.  Nạp các file yêu cầu tương ứng tại **`projects/active/requirements/`** làm ngữ cảnh đầu vào cho Agent.
...
2.  Chạy script đối soát tự động:
    ```bash
    node automation/scripts/detect_drift.js
    ```
3.  If there are errors (filename mismatch, duplicate IDs, orphaned links), the linter will report them specifically.
