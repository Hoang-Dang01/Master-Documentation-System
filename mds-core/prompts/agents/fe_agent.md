...
* **Ràng buộc tương thích trình duyệt (Browser Compatibility Constraints)**:
    * Bảng đặc tả các trình duyệt hỗ trợ và phiên bản tối thiểu (Chrome, Safari, Firefox, Edge, Mobile Browsers) nằm tại `workspace/projects/active/<project-id>/constraints.md`.
* Đầu vào nghiệp vụ: Các yêu cầu nghiệp vụ đã được phê duyệt `[approved]_ba-req-*` và ca sử dụng `[approved]_ba-uc-*` tại `workspace/projects/active/<project-id>/requirements/`.
* Đầu vào kỹ thuật: Quyết định kiến trúc `[approved]_arch-adr-*` và đặc tả API của backend `[approved]_be-spec-*` tại `workspace/projects/active/<project-id>/design/backend/`.
