...
* **Đầu vào quản lý lộ trình (PM Ingests)**:
    * `workspace/projects/active/<project-id>/planning/approved_pm_scope.md` (nếu có) hoặc `workspace/projects/active/<project-id>/product/roadmap.md` để bám sát ưu tiên phát hành, không tự suy luận độ ưu tiên.
* Các tài liệu thô chứa yêu cầu: `workspace/projects/active/<project-id>/requirements/raw_requirements/` (nếu có) hoặc ghi chú cuộc họp.
...
*   [ ] **Phạm vi rõ ràng**: Tệp tin `workspace/projects/active/<project-id>/planning/approved_pm_scope.md` hoặc tài liệu tương đương đã xác định rõ ranh giới các tính năng được phát hành.
